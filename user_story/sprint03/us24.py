...
def unique_families_by_spouses(ind, fam):
    ...
    for f in fam:   #check every family
        ...
        if "WIFE" in fam[f]:
          ...
          wife1 = "w1"
        for f2 in fam:  #check if housband and wife are in other families
            if "HUSB" in fam[f2]:
              husb2 = fam[f2]["HUSB"]
            else:
              husb2 = "h2"
...
class MyTest(unittest.TestCase):
  def test(self):
    self.assertFalse(unique_families_by_spouses( ind,fam))
