def userStory42(result):
    """print out all the illegal date, with the line of record in the document
    """
...
def dateVerify(checkdate):
    ...
    if date[0] == '30' and date[1] == 'FEB':
        res = False
    if date[0] == '29' and date[1] == 'FEB' and int(date[2]) % 4 != 0: 
        res = False
    if date[0] == '29' and date[1] == 'FEB' and ((int(date[2]) % 100 == 0 and int(date[2]) % 400 != 0)):
        res = False
    
    return res

print(dateVerify('29FEB2000'))